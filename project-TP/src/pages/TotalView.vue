<template>
  <div class="containerhome">
    ...
    <h3>
      수입 / 지출 전체 조회
      <i @click="refreshPage" class="fa-solid fa-rotate-right"></i>
    </h3>
    <!--필터링 옵션 영역-->
...
export default {
  setup() {
    const store = useAccountListStore(); // pinia 스토어 사용
    const router = useRouter(); // 라우터 사용
    const { state, fetchLists, filterLists } = store; // 스토어에서 상태와 메서드 추출

    const isArrowVisible = ref(true); // 금액 오름차순, 내림차순 flag

    const toggleArrow = () => {
      // 금액 기준으로 오름차순, 내림차순 정렬
